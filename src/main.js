...
const agreementText = fs.readFileSync('agreement.txt', 'utf-8');
...
const aiResponse = await llm.invoke([
    {
        ...
        These should be provided in the form of a JSON like the following with only the JSON present and nothing else in the response. If there is something that cannot be deciphered, just leave it as "undefined". DO NOT ADD ANYTHING OUTSIDE THE JSON:
        {
            "draftedBy": "John Doe",
            "draftedFor": "Jane Doe",
...
const draftedBy = jsonResponse.draftedBy;
const draftedFor = jsonResponse.draftedFor;
const description = jsonResponse.description;
const nonNegotiableClauses = jsonResponse.nonNegotiableClauses;
const deadlines = jsonResponse.deadlines;
const termsAndConditions = jsonResponse.termsAndConditions;

console.log("Drafted By:", draftedBy);
console.log("Drafted For:", draftedFor);
console.log("Description:", description);
console.log("Non-Negotiable Clauses:", nonNegotiableClauses);
